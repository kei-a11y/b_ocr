...
#pytesseract.pytesseract.tesseract_cmd = r"C:\Program Files\Tesseract-OCR\tesseract.exe" #デプロイ時は削除？
...
def remove_colon_and_noise(pil_img, debug_name=""):
    ...
    #---「画像のヒストグラム（二値化前の明るさ分布）を解析して、クラス内分散を最小にするしきい値」を自動的に決める手法
    #--- つまり、明るい背景と暗い文字がある時、それらを最もきれいに分ける閾値（0〜255の数値）を自動で計算してくれる
...
def ocr_time_with_fallback(pil_img, model=None, debug_name=""):
    ...
    return ""
    ### ocr_time_with_fallback の返り値は
    ### Tesseract が成功した場合 → norm
    ### Tesseract が失敗した場合（３桁、４桁以外）→ valをさらに正規化したもの
    ### 元々の分岐コード　len(norm) != 4 and model is not None:
    ###### ↑これだと無理してTesseractで処理してしまってmidに途中処理されない
